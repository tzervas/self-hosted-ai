...
class ADKAgent(ABC):
    ...
    async def _call_litellm(self, messages: List[Dict[str, str]], **kwargs) -> Dict[str, Any]:
        ...
        response = await self.client.post(
            f"{self.config.litellm_url}/v1/chat/completions", json=payload, headers=headers
        )
    ...
    async def execute_streaming(
        ...
        async with self.client.stream(
            "POST", f"{self.config.litellm_url}/v1/chat/completions", json=payload, headers=headers
        ) as response:
