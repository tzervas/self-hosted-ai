...
def extract_navigation_matrix(content: str) -> Dict[str, Any]:
    ...
    matrix = {"by_persona": [], "by_task": [], "by_urgency": []}
        ...
        for row in rows:
            ...
            if len(cells) >= 3:
                matrix["by_persona"].append(
                    {"persona": cells[0].strip("*"), "start_here": cells[1], "then_read": cells[2]}
                )
        ...
        documents.append(
            {
                ...
                "token_variance": abs(actual_tokens - claimed_tokens) / claimed_tokens
                if claimed_tokens
                else 0,
...
def extract_quick_references(content: str) -> Dict[str, List[str]]:
    ...
    refs = {"commands": [], "endpoints": [], "sync_waves": [], "namespaces": []}

    # Extract common commands
    cmd_section = re.search(r"### Common Commands\s*```bash\s*\n(.*?)\n```", content, re.DOTALL)
    if cmd_section:
    ...
    refs["endpoints"] = [
        {"service": m.group(1), "url": m.group(2)} for m in re.finditer(endpoint_pattern, content)
    ]
        ...
        for row in rows:
            ...
            if len(cells) >= 3:
                refs["sync_waves"].append(
                    {"wave": cells[0], "services": cells[1], "purpose": cells[2]}
                )
...
def extract_emergency_procedures(content: str) -> Dict[str, List[str]]:
    ...
    emergency_section = re.search(r"## 🚨 Emergency Procedures.*?(?=##|\Z)", content, re.DOTALL)
    ...
    emergency_types = ["Production Service Down", "Deployment Failing", "Security Incident"]
...
def compute_statistics(documents: List[Dict], project_root: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "token_accuracy": 1.0
        - (abs(total_actual_tokens - total_claimed_tokens) / total_claimed_tokens)
        if total_claimed_tokens
        else 1.0,
