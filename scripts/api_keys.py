...
class KeyConfig:
    ...
    database_url: str = field(
        default_factory=lambda: os.environ.get(
            "DATABASE_URL", "postgresql://agents:agents@localhost:5432/agents"
        )
    )
    rotation_days: int = field(
        default_factory=lambda: int(os.environ.get("KEY_ROTATION_DAYS", "90"))
    )
    warning_days: int = field(default_factory=lambda: int(os.environ.get("KEY_WARNING_DAYS", "80")))
    ...
    async def list_keys(self) -> list[APIKey]:
        ...
        return [
            APIKey(
                ...
                rotated_at=row["rotated_at"].replace(tzinfo=timezone.utc)
                if row["rotated_at"]
                else None,
