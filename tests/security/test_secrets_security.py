...
class TestNoPlaintextSecrets:
            ...
            for pattern, desc in self.SECRET_PATTERNS:
                ...
                for match in matches:
                    ...
                    if "${" in match or "{{" in match:
                        continue
                    violations.append(f"{values_file.relative_to(PROJECT_ROOT)}: {desc}")

        assert not violations, f"Potential hardcoded secrets in Helm values:\n" + "\n".join(
    ...
    def test_no_plaintext_in_sealed_secrets(self):
        ...
        for secret_file in secrets_dir.glob("*.yaml"):
            ...
            if kind == "SealedSecret":
                ...
                plaintext.append(f"{secret_file.name}: plain Secret (should be SealedSecret)")
    ...
    def test_no_env_files_committed(self):
        ...
        assert not actual_env_files, f".env files found in active directories:\n" + "\n".join(
            f"  - {f.relative_to(PROJECT_ROOT)}" for f in actual_env_files
        )
    ...
    def test_gitignore_excludes_secrets(self):
        ...
        if missing:
            pytest.xfail(f".gitignore missing patterns: {missing}")
