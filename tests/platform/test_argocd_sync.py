...
class TestArgocdApplications:
    ...
    def test_apps_sync_status(self, argocd_apps):
        ...
        assert not problems, f"Critical applications with unknown sync status:\n" + "\n".join(
    ...
    def test_apps_health_status(self, argocd_apps):
        ...
        for app in argocd_apps["items"]:
            ...
            if health_status == "Degraded":
                message = app.get("status", {}).get("health", {}).get("message", "no message")
    ...
    def test_critical_apps_healthy(self, argocd_apps):
        ...
        assert not unhealthy, f"Unhealthy critical applications:\n" + "\n".join(
            f"  - {u}" for u in unhealthy
        )
    ...
    def test_no_operation_errors(self, argocd_apps):
        ...
        assert not errors, f"ArgoCD operation errors:\n" + "\n".join(f"  - {e}" for e in errors)
