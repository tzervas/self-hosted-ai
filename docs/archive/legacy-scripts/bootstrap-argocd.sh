...
if [[ "$SKIP_ARGOCD" == "false" ]]; then
    ...
    helm upgrade --install argocd argo/argo-cd \
        ...
        --set server.ingress.hosts[0]=argocd.vectorweight.com \
        --set server.ingress.tls[0].hosts[0]=argocd.vectorweight.com \
        --set server.ingress.tls[0].secretName=argocd-vectorweight-tls \
        --set server.ingress.annotations."cert-manager\.io/cluster-issuer"=letsencrypt-production \
    ...
    echo "├─────────────────────────────────────────────────────────────────────────┤"
    echo "│ URL:      https://argocd.vectorweight.com (or port-forward below)       │"
    echo "│ Username: admin                                                          │"
